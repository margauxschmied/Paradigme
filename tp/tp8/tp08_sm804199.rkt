...
(define-type Cont
  ...
  [whileCndK (c : Exp) (e : Exp) (env : Env) (k : Cont)]
...
(define (interp [e : Exp] [env : Env] [sto : Store] [k : Cont]) : Value
  (type-case Exp e
    ...
    [(whileE cnd e) (interp cnd env sto (whileCndK cnd e env k))]
...
(define (escape [k : Cont] [val : Value] sto) : Value
  (type-case Cont k
    ...
    [(whileCndK c e env next-k) (error 'interp "break outside while")]    
    ))  
...
(define (continue [k : Cont] [val : Value] [sto : Store]) : Value
  (type-case Cont k
    ...
    [(whileDoK c e env next-k)
     (interp c env sto (whileCndK c e env next-k))]   
                         
    [(whileCndK c e env next-k) 
     (if (equal? val (numV 0))  
