...
(define (interp [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(letrecE s rhs body)
     ...
     (let ([new-env-end (new-enve s env)]) 
       (letrece s rhs body new-env-end))] 
    [(ifE cnd l r)
...
(define (cherche-box e env)
  (if (empty? env)
      (error 'interp "free identifier")
      (if (equal? (bind-name (first env)) e)
          (bind-val (first env))
          (cherche-box e (rest env)))))
          
           
 
(define (letrece s rhs body env)
    (if (not (empty? rhs))  
        (begin
          (set-box! (cherche-box (first s) env) (delay (first rhs) env (box (none))))
          (letrece (rest s) (rest rhs) body env))
        ;(interp (letrecE s (rest rhs) body) new-env-end))
        (interp body env))) 

(define (new-enve s env)
  (if (empty? s)
      env
      (let ([b (box (undef))])
        (let ([new-env-end (extend-env (bind (first s) b) env)])
          (new-enve (rest s) new-env-end)))))
...
(test (interp-expr `(letrec ((x x)) (letrec ((a b)) x))) (undefV))