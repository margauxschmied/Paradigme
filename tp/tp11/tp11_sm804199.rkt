...
(define (typecheck [e : Exp] [env : TypeEnv]) : Type 
  (type-case Exp e
    ...
    [(getE rec fd)
     (type-case Type (typecheck rec env)
       [(recordT fds vs) (find fd fds vs)]
       [else (error 'interp "not a record")])]  
    [(setE rec fd arg)  
     (type-case Type (typecheck rec env) 
       [(recordT fds vs) (if (is-subtype? (typecheck arg env) (find fd fds vs))
                             (typecheck rec env);(recordT fds (updateType fd (typecheck arg env) fds vs))
                             (error 'interp "not good type"))]
       [else (error 'interp "not a record")])]))


(define (is-subtype? [t1 : Type] [t2 : Type]) : Boolean  ;t1>t2
  (cond
    [(and (recordT? t1) (recordT? t2))
     (enregitrement (recordT-fields t2) (recordT-fields t1) (recordT-type-fields t1) (recordT-fields t2) (recordT-type-fields t2))]
...
;      #t
;      (type-case Type t1
;        [(recordT fds1 vs1) (type-case Type t2
;                              [(recordT fds2 vs2) (enregitrement fds1 fds1 vs1 fds2 vs2)]
;                              [else (error 'interp "not a record")])]
;        [else (error 'interp "not a record")])
;      )
  )  

(define (enregitrement fds fds1 vs1 fds2 vs2)  
  (if (empty? fds) 
      #t
      ;      (type-case Type (find (first fds) fds1 vs1)
      ;       [(recordT fds3 vs3) (if (is-subtype? (find (first fds) fds1 vs1) (find (first fds) fds2 vs2))
      ;                             (enregitrement (rest fds) fds1 vs1 fds2 vs2)
      ;                             #f)] 
      ;      [else (if (equal? (find (first fds) fds1 vs1) (find (first fds) fds2 vs2))
      ;          (enregitrement (rest fds) fds1 vs1 fds2 vs2)
      ;          #f)])
      (if (and (exist (first fds) fds1) (is-subtype? (find (first fds) fds1 vs1) (find (first fds) fds2 vs2)))
          (enregitrement (rest fds) fds1 vs1 fds2 vs2)
          #f)))
...
(define (exist fd fds)
  (cond
    ...
    [else (exist fd (rest fds))]))


(define (updateType [fd : Symbol] [new-val : Type]
                    [fds : (Listof Symbol)] [vs : (Listof Type)]) : (Listof Type)
  (cond
    [(empty? fds) (error 'interp "no such field")]
    [(equal? fd (first fds)) (cons new-val (rest vs))]
    [else (cons (first vs) (updateType fd new-val (rest fds) (rest vs)))]))
...
(define (find [fd : Symbol] [fds : (Listof Symbol)] [vs : (Listof 'a)]) : 'a
  (cond
    [(empty? fds) (begin (display fd) (error 'interp "no such field"))] 
